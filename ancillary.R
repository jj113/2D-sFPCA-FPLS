FPLS_img = function(type,img.list,npc,timepts=NULL,ncr){
  if (is.null(timepts)) {
    timepts1=1:ncr
    timepts2=1:ncr
  }
  ...
  XB = NULL # project data onto the B-spline basis
  for(i in 1:nrow(train_dat.id)){
    XB = rbind(XB, as.vector(t(desMat) %*% as.vector(img.list[[i]])))
  }
  
  return(list(desMat, XB))
}
